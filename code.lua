...
local function CreateESP(Player)
    local Box = Drawing.new("Square"); local DistText = Drawing.new("Text")
    RunService.RenderStepped:Connect(function()
        if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") and Player ~= LocalPlayer then
            ...
            if OnScreen then
                ...
                if _G.ESP_Distance then
                    ...
                    DistText.Position = Vector2.new(Pos.X, Pos.Y + (2000 / Pos.Z / 2))
                    DistText.Text = d .. "m"; DistText.Visible = true; DistText.Center = true; DistText.Outline = true
                else DistText.Visible = false end
            else Box.Visible = false; DistText.Visible = false end
        else Box.Visible = false; DistText.Visible = false end
    end)
end
